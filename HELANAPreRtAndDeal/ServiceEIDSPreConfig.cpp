...
std::string ServiceEIDSPreConfig::getOciSessionName(void)
{
    std::string strSessionName = 
        m_pConfigOpt->GetValueByKey(ORACLE_CONFIG, ORACLE_SESSION_NAME_KEY);
...
std::string ServiceEIDSPreConfig::getOciUserName(void)
{
    std::string strUserName = 
        m_pConfigOpt->GetValueByKey(ORACLE_CONFIG, ORACLE_USER_NAME_KEY);
...
std::string ServiceEIDSPreConfig::getOciPassWD(void)
{
    std::string strPassWD = 
        m_pConfigOpt->GetValueByKey(ORACLE_CONFIG, ORACLE_PASSWD_KEY);
    if (strPassWD.empty())
    {
...
{
	std::string strType = 
		m_pConfigOpt->GetValueByKey(ORACLE_CONFIG, ORACLE_TYPE);
	if (strType.empty())
	{
...
std::string ServiceEIDSPreConfig::getRsdbIp(void)
{
	std::string strReturn = 
		m_pConfigOpt->GetValueByKey(ORACLE_CONFIG, IP);
	return strReturn;
}
std::string ServiceEIDSPreConfig::getRsdbPort(void)
{
	std::string strReturn = 
		m_pConfigOpt->GetValueByKey(ORACLE_CONFIG, PORT);
...
int 
ServiceEIDSPreConfig::getServiceCalCycSec()
{
