...
{
//	otl_lob_stream lobPointList,lobMaValue,lobModelValue,lobModelValueRelation;
	//otl_long_string lstr(1024*1024);
	//otl_long_string lstr(20000);
...
//	DataMode* mode_info;
	...
	//Aos_Assert_S("TbPubModeMethodAvg::loadValue**")
	...
	while (mQtOpt->SQLStmtFetch())
	{
		...
		//Aos_Assert_S("TbPubModeMethodAvg::loadValue**1")
		...
		iter->second->m_IsDeal=true;
		iter->second->mRtdbSimModleIsWrite=true;*/
        ...
        nErr = Util::StringSplit(strPointList,condtion_fun->mVectorModePoint, "@");   
        if(!nErr) 
        {
            condtion_fun->mIsOk=false;
  ...
  //              }
	...
	Aos_WriteLog(PubOpt::StringOpt::StringFormat("Load %s Info Total: %d",
		MODEFUN_CONFIG, pModeFunInfo.size()).c_str());
	return true;
}
