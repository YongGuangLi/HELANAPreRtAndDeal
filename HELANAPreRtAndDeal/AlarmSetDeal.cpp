...
bool AlarmSetDeal::RsltPointInToRsdb()
{
	...
	for (;it!= mMapAlarmPointRst.end();++it)
	{
		...
		/*if("4_zc"!=rslt_point->mStrModelId)
			continue;*/
		//Aos_WriteLog((PubOpt::StringOpt::StringFormat("%s", rslt_point->mStrPointId.c_str())).c_str());
		//if (rslt_point->mStrPointId=="fdj_fdjzz_20MKD12CT113B"||rslt_point->mStrPointId=="fdj_fdjzz_20MKD11CY122")
		//{
		//	Aos_WriteLog("**************************************************");
		//	Aos_WriteLog((PubOpt::StringOpt::StringFormat("%s", rslt_point->mStrPointId.c_str())).c_str());
		//	//Aos_WriteLog((Util::Number2String(rslt_point->m_savgValue)).c_str());
		//	//int i =100;
		//}
		//Aos_WriteLog(rslt_point->mStrPointId.c_str());
		if ((rslt_point->mDate->m_curValue ==rslt_point->mDate->m_lastValue
			&&rslt_point->mDate->m_isInto
			&&rslt_point->mDate->m_TimeLong >=rslt_point->mDate->m_deaBand)
			||(rslt_point->mDate->m_curValue !=rslt_point->mDate->m_lastValue
			...
			&&rslt_point->mDate->m_LastTimeLong >=rslt_point->mDate->m_deaBand))
		{
			...
			{
				Aos_WriteLog("**************************************************");
				Aos_WriteLog((PubOpt::StringOpt::StringFormat("RsltPointInToRsdb 模型：%s 点名：%s guid：%s", 
					rslt_point->mStrModelId.c_str(),rslt_point->mStrPointId.c_str(),guId.c_str())).c_str());
			}*/
			/*if (rslt_point->mStrPointId=="mmj_4mmjbt_20HFC40CT001"||rslt_point->mStrPointId=="mmj_5mmjbt_20HFC50CT001")
			{
				Aos_WriteLog((PubOpt::StringOpt::StringFormat("$$RsltPointInToRsdb 模型：%s 点名：%s guid：%s m_savgValue:%f", 	                                             
...
bool AlarmSetDeal::RsltPointUpBackInRsdb()
{
	...
	for (;it!= mMapAlarmPointRst.end();++it)
	{
			...
			&&(!rslt_point->mDate->m_lastisInto))
		{
			...
			{
				Aos_WriteLog("**----------------------------------**");
				Aos_WriteLog((PubOpt::StringOpt::StringFormat("RsltPointUpBackInRsdb 模型：%s 点名：%s guid：%s endTime：%s", 
					rslt_point->mStrModelId.c_str(),rslt_point->mStrPointId.c_str(),guId.c_str(),endTime.c_str())).c_str());
			}*/
			listEndTime<<QString(endTime.c_str());
			if (rslt_point->mIsGetCurVale)
			{
