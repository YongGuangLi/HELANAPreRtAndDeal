...
if("ZZ2_3_fl_1gjsdc"==strGroup)	
 int itep =100;
		ValueType = mQtOpt->m_query->value(21).toInt();
		ValueNum = mQtOpt->m_query->value(22).toInt();
		//ValueType = std::string(mQtOpt->m_query->value(20).toString().toLocal8Bit());
		//ValueNum = std::string(mQtOpt->m_query->value(21).toString().toLocal8Bit());
		...
		/*aMax = mQtOpt->m_query->value(22).toDouble();
		aMin = mQtOpt->m_query->value(23).toDouble();
		aCap = mQtOpt->m_query->value(24).toDouble();
		aSource = mQtOpt->m_query->value(25).toDouble();
		aValue = mQtOpt->m_query->value(26).toDouble();*/
		if (strSet.empty()) 
		{
			Aos_Assert_S("机组编号是空");
			continue; 
		...
		if (iter==syscf->mMapModles.end())
		{
			...
			/*v_it = std::find(VectorModleName.begin(), VectorModleName.end(), strModel);
			if(v_it == VectorModleName.end())
				VectorModleName.push_back(strModel);*/
		...
		if (c_iter==mode_info->mMapModeCond.end())
		{
			//setOutVarAndFunc(mode_condtion->m_strOutVar,mVectorConPoint);
...
{
	std::string strModel,strCode,strSouce,strSet,strFullCode;
	MapStringToString_It p_itr;
	if(mQtOpt->QtGetResultIsValid())
		return false;
	int nNumber = 0;
	while (mQtOpt->SQLStmtFetch())
